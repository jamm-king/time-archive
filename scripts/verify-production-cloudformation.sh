...
log() {
  printf '[verify-production-cloudformation] %s\n' "$1"
}

fail() {
  printf '[verify-production-cloudformation] ERROR: %s\n' "$1" >&2
  exit 1
}
...
TEMPLATE="$ROOT_DIR/infra/cloudformation/production.yml"
PARAMETERS="$ROOT_DIR/infra/cloudformation/production.parameters.example.json"
...
def validate(template):
    ...
    expected_master_password_path = "/time-archive/bootstrap/production/database/master-password"
    ...
    if not str(master_password_parameter.get("AllowedPattern", "")).startswith(
        "^/time-archive/bootstrap/production/"
    ):
    ...
    deploy_trust = (
        resources.get("GitHubProductionDeployRole", {})
        .get("Properties", {})
        .get("AssumeRolePolicyDocument", {})
        .get("Statement", [{}])[0]
        .get("Condition", {})
    ...
    if not sub_contains(
        ...
        errors.append("GitHub deploy role trust must be limited to the production environment")
    ...
    if "ecr:DescribeImages" not in deploy_actions:
        errors.append("GitHub deploy role must verify production image tags before deployment")
    ...
    if describe_image_resources != [
        ...
        errors.append("GitHub deploy role ECR verification must be limited to production repositories")
...
mutated["Parameters"]["DatabaseMasterPasswordParameterName"]["Default"] = (
    "/time-archive/production/database/master-password"
)
if not validate(mutated):
    raise SystemExit("policy self-test failed to detect a runtime-readable database master password")

mutated = copy.deepcopy(template)
mutated["Resources"]["Database"]["Properties"]["DeletionProtection"] = False
if not validate(mutated):
    raise SystemExit("policy self-test failed to detect disabled production RDS deletion protection")

mutated = copy.deepcopy(template)
mutated["Resources"]["Database"]["Properties"]["BackupRetentionPeriod"] = 1
if not validate(mutated):
    raise SystemExit("policy self-test failed to detect insufficient production RDS backup retention")

print("production CloudFormation architecture policy validation passed")
PY

log "Production CloudFormation validation passed"
