...
log() {
  printf '[verify-staging-presigned-upload-cors-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-staging-presigned-upload-cors-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-staging-presigned-upload-cors.yml"
SCRIPT="$ROOT_DIR/scripts/verify-staging-presigned-upload-cors-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "staging-presigned-upload-cors-smoke":
        errors.append("workflow must serialize the staging presigned upload CORS smoke group")
    ...
    for required in (
        "verify-staging-presigned-upload-cors-smoke.sh",
    ...
    for required in (
        ...
        "staging-presigned-upload-cors-smoke.png",
    ):
        if required not in script_text:
            errors.append(f"script is missing required CORS smoke behavior: {required}")

    for forbidden in ("/complete", "/approve", "/reject", "/hide", "/api/internal/payments/fake"):
        if forbidden in script_text:
            errors.append(f"script must not exercise forbidden endpoint: {forbidden}")
    for forbidden in ("upload_url", "original_file_url"):
...
mutated_script = script_text.replace("Access-Control-Request-Method: PUT", "Access-Control-Request-Method: GET")
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect missing PUT preflight")

mutated_script = script_text + "\ncurl /api/owned-ranges/$ownership_record_id/media/upload-requests/$upload_request_id/complete\n"
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect forbidden completion endpoint")

print("staging presigned upload CORS smoke workflow policy validation passed")
PY

log "Staging presigned upload CORS smoke workflow validation passed"
