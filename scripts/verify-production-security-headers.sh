...
log() {
  printf '[verify-production-security-headers] %s\n' "$1"
}

fail() {
  printf '[verify-production-security-headers] ERROR: %s\n' "$1" >&2
  exit 1
}

usage() {
  cat <<'USAGE'
Usage: scripts/verify-production-security-headers.sh [--base-url URL]

Verifies security headers through the deployed public production HTTPS hostname.
...
detect_python() {
  ...
  if command -v python >/dev/null 2>&1 && python -c 'import re' >/dev/null 2>&1; then
    printf 'python'
    return
  fi
...
log "Production security headers check passed"
