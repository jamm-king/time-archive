...
log() {
  printf '[grant-production-owned-range] %s\n' "$1"
}

fail() {
  printf '[grant-production-owned-range] ERROR: %s\n' "$1" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: scripts/grant-production-owned-range.sh --expected-account-id ID --email EMAIL --start-second N --end-second N [options]

Options:
  --expected-account-id ID  Required 12-digit AWS account ID.
  --email EMAIL             Existing production user email to receive the range.
...
The script creates production-only ADMIN_GRANT ownership data. It does not touch
...
[[ "$AWS_REGION_VALUE" == "ap-northeast-2" ]] ||
  fail "production owned range grant must run in ap-northeast-2"
...
log() {
  printf '[remote-grant-production-owned-range] %s\n' "\$1"
}

fail() {
  printf '[remote-grant-production-owned-range] ERROR: %s\n' "\$1" >&2
  exit 1
}
...
log "Granting ADMIN_GRANT ownership range to existing production user"
...
log "production owned range grant completed"
REMOTE
...
payload = {
    ...
    "Comment": "Grant Time Archive production owned range",
    "Parameters": {
...
log "production owned range grant succeeded"
