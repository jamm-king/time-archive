...
log() {
  printf '[verify-staging-media-duration-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-staging-media-duration-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-staging-media-duration.yml"
SCRIPT="$ROOT_DIR/scripts/verify-staging-media-duration-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "staging-media-duration-smoke":
        errors.append("workflow must serialize the staging media duration smoke group")
    if as_bool(concurrency.get("cancel-in-progress")):
        errors.append("workflow must not cancel an in-progress media duration smoke check")
    ...
    for required in (
        "verify-staging-media-duration-smoke.sh",
        "INPUT_PUBLIC_BASE_URL",
        "INPUT_START_SECOND",
        "INPUT_END_SECOND",
        "STAGING_PUBLIC_BASE_URL",
        "Short video upload completion",
        "Over-duration video rejection",
        "GITHUB_STEP_SUMMARY",
    ):
        if required not in step_text:
            errors.append(f"workflow is missing required media duration smoke behavior: {required}")
    ...
    for required in (
        ...
        "/api/auth/login",
        "/api/me/owned-ranges",
        "/api/owned-ranges/$ownership_record_id/media",
        "/api/owned-ranges/$ownership_record_id/media/upload-requests",
        "MEDIA_DURATION_EXCEEDS_OWNED_RANGE",
        "staging-media-duration-short.mp4",
        "staging-media-duration-too-long.mp4",
        "durationMs",
        "owned_media_count_after_long == owned_media_count_after_short",
        "STAGING_ADMIN_EMAIL",
        "STAGING_ADMIN_PASSWORD",
    ):
        if required not in script_text:
            errors.append(f"script is missing required media duration smoke behavior: {required}")
    ...
    for forbidden in ("upload_url", "short_upload_url", "long_upload_url"):
...
mutated_script = script_text.replace("MEDIA_DURATION_EXCEEDS_OWNED_RANGE", "INVALID_REQUEST")
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect missing duration rejection assertion")
...
print("staging media duration smoke workflow policy validation passed")
PY

log "Staging media duration smoke workflow validation passed"
