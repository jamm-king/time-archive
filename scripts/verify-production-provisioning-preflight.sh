...
log() {
  printf '[verify-production-provisioning-preflight] %s\n' "$1"
}

fail() {
  printf '[verify-production-provisioning-preflight] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SCRIPT="$ROOT_DIR/scripts/verify-production-provisioning-inputs.sh"
FIXTURE="$ROOT_DIR/infra/cloudformation/production.parameters.test.json"
...
case "$1 $2" in
  ...
  "ssm describe-parameters")
    [[ " $* " == *"/time-archive/bootstrap/production/database/master-password"* ]] || exit 2
...
log "Production provisioning preflight policy validation passed"
