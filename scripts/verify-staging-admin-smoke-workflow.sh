...
log() {
  printf '[verify-staging-admin-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-staging-admin-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-staging-admin.yml"
SCRIPT="$ROOT_DIR/scripts/verify-staging-admin-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "staging-admin-smoke":
        errors.append("workflow must serialize the staging admin smoke group")
...
print("staging admin smoke workflow policy validation passed")
PY

log "Staging admin smoke workflow validation passed"
