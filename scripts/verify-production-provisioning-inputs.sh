...
log() {
  printf '[verify-production-provisioning-inputs] %s\n' "$1"
}

fail() {
  printf '[verify-production-provisioning-inputs] ERROR: %s\n' "$1" >&2
  exit 1
}
...
[[ "$REGION" == "ap-northeast-2" ]] || fail "Production region must be ap-northeast-2"
...
for key in (
    ...
    print(values[key])
PY
) || fail "Local production parameter validation failed"
...
log "Local production parameter validation passed"
...
PASSWORD_PARAMETER_TYPE="$(aws ssm describe-parameters \
  --parameter-filters '[{"Key":"Name","Option":"Equals","Values":["/time-archive/bootstrap/production/database/master-password"]}]' \
  --query 'Parameters[0].Type' \
  --output text \
  --region "$REGION")"
PASSWORD_PARAMETER_TYPE="${PASSWORD_PARAMETER_TYPE%$'\r'}"
[[ "$PASSWORD_PARAMETER_TYPE" == "SecureString" ]] || fail "Infrastructure-only production database master password parameter must exist as SecureString"
...
log "Production provisioning preflight passed; no AWS resources were changed"
