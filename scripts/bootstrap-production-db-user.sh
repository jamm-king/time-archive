...
log() {
  printf '[bootstrap-production-db-user] %s\n' "$1"
}

fail() {
  printf '[bootstrap-production-db-user] ERROR: %s\n' "$1" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: scripts/bootstrap-production-db-user.sh --expected-account-id ID [options]
...
Options:
  ...
  --stack-name NAME         CloudFormation stack. Defaults to time-archive-production.
...
AWS_PROFILE_VALUE=""
STACK_NAME="time-archive-production"
DRY_RUN=false
ALLOW_TEMPORARY_MASTER_PASSWORD_READ=false
TEMPORARY_POLICY_ATTACHED=false
APPLICATION_ROLE_NAME=""
TEMPORARY_POLICY_NAME="time-archive-production-db-user-bootstrap-master-read"
...
[[ "$AWS_REGION_VALUE" == "ap-northeast-2" ]] ||
  fail "Production database bootstrap must run in ap-northeast-2"
...
[[ "$RUNTIME_PARAMETER_PATH" == "/time-archive/production/" ]] ||
...
log() {
  printf '[remote-bootstrap-production-db-user] %s\n' "\$1"
}

fail() {
  printf '[remote-bootstrap-production-db-user] ERROR: %s\n' "\$1" >&2
  exit 1
}
...
BOOTSTRAP_MASTER_PASSWORD_PARAMETER="/time-archive/bootstrap/production/database/master-password"
APP_USERNAME_PARAMETER="/time-archive/production/database/username"
APP_PASSWORD_PARAMETER="/time-archive/production/database/password"
...
[[ "\$APP_USERNAME" == "timearchive_prod_app" ]] || fail "Unexpected app database username"
...
grant connect on database time_archive to timearchive_prod_app;
grant usage, create on schema public to timearchive_prod_app;
grant select, insert, update, delete on all tables in schema public to timearchive_prod_app;
grant usage, select, update on all sequences in schema public to timearchive_prod_app;
alter default privileges in schema public
  grant select, insert, update, delete on tables to timearchive_prod_app;
alter default privileges in schema public
  grant usage, select, update on sequences to timearchive_prod_app;
SQL

log "Pulling PostgreSQL client image"
docker pull postgres:18-alpine >/dev/null

log "Applying production database role and grants"
...
log "Verifying production database role login"
docker run --rm --network host \\
...
log "Production database user bootstrap completed"
REMOTE
...
payload = {
    ...
    "Comment": "Bootstrap Time Archive production database user",
    "Parameters": {
    ...
    "Statement": [
        {
            ...
            "Resource": f"arn:aws:ssm:ap-northeast-2:{account_id}:parameter/time-archive/bootstrap/production/database/master-password",
        }
    ],
}
with open(output_path, "w", encoding="utf-8") as target:
...
log "Production database user bootstrap succeeded"
