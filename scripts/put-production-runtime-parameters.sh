...
log() {
  printf '[put-production-runtime-parameters] %s\n' "$1"
}

fail() {
  printf '[put-production-runtime-parameters] ERROR: %s\n' "$1" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: scripts/put-production-runtime-parameters.sh --expected-account-id ID [options]

Options:
  --input FILE              Local parameter input file.
                            Defaults to deploy/production/runtime-parameters.local.json.
  ...
  --allow-paypal-enabled    Allow /paypal/enabled=true after explicit launch approval.
  --help                    Show this help.
...
INPUT_FILE="$ROOT_DIR/deploy/production/runtime-parameters.local.json"
CONTRACT_FILE="$ROOT_DIR/deploy/production/ssm-parameters.example.json"
AWS_REGION_VALUE="ap-northeast-2"
AWS_PROFILE_VALUE=""
EXPECTED_ACCOUNT_ID=""
DRY_RUN=false
VALIDATE_ONLY=false
ALLOW_PAYPAL_ENABLED=false
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --dry-run)
      ...
      shift
      ;;
    --allow-paypal-enabled)
      ALLOW_PAYPAL_ENABLED=true
      shift
      ;;
    --validate-only)
      VALIDATE_ONLY=true
...
[[ "$AWS_REGION_VALUE" == "ap-northeast-2" ]] ||
  fail "Production runtime parameters must be written in ap-northeast-2"
...
"$PYTHON_BIN" - "$CONTRACT_FILE" "$INPUT_FILE" "$VALIDATED_PARAMETERS_FILE" "$ALLOW_PAYPAL_ENABLED" <<'PY'
import json
import sys

contract_path, input_path, output_path, allow_paypal_enabled_value = sys.argv[1:5]
allow_paypal_enabled = allow_paypal_enabled_value == "true"
...
for name in sorted(expected):
    ...
    if "/staging/" in name or "staging" in value.lower():
        raise SystemExit(f"production input must not reference staging: {name}")
    if name == "/time-archive/production/aws/region" and value != "ap-northeast-2":
        raise SystemExit("production AWS region must be ap-northeast-2")
    if name == "/time-archive/production/cloudwatch/log-group-prefix" and value != "/time-archive/production":
        raise SystemExit("production CloudWatch log group prefix must be /time-archive/production")
    if name == "/time-archive/production/paypal/api-base-url" and value != "https://api-m.paypal.com":
        raise SystemExit("production PayPal API base URL must be https://api-m.paypal.com")
    if name == "/time-archive/production/paypal/return-url" and value != "https://time-archive.com/payments/paypal/return":
        raise SystemExit("production PayPal return URL must be https://time-archive.com/payments/paypal/return")
    if name == "/time-archive/production/paypal/cancel-url" and value != "https://time-archive.com/payments/paypal/cancel":
        raise SystemExit("production PayPal cancel URL must be https://time-archive.com/payments/paypal/cancel")
    if name == "/time-archive/production/paypal/enabled" and value not in {"false", "true"}:
        raise SystemExit("production PayPal enabled must be true or false")
    if name == "/time-archive/production/paypal/enabled" and value == "true" and not allow_paypal_enabled:
        raise SystemExit(
            "keep production PayPal disabled during initial runtime provisioning; "
            "use --allow-paypal-enabled only after explicit launch approval"
        )
    if name == "/time-archive/production/rate-limit/client-ip-header" and value == "":
        continue
...
print(f"validated {len(validated)} production runtime parameters")
PY
...
log "Production runtime parameters written"
