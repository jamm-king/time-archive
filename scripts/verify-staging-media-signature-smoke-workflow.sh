...
log() {
  printf '[verify-staging-media-signature-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-staging-media-signature-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-staging-media-signature.yml"
SCRIPT="$ROOT_DIR/scripts/verify-staging-media-signature-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "staging-media-signature-smoke":
        errors.append("workflow must serialize the staging media signature smoke group")
    if as_bool(concurrency.get("cancel-in-progress")):
        errors.append("workflow must not cancel an in-progress media signature smoke check")
    ...
    for required in (
        "verify-staging-media-signature-smoke.sh",
        "INPUT_PUBLIC_BASE_URL",
        "INPUT_START_SECOND",
        "INPUT_END_SECOND",
        "STAGING_PUBLIC_BASE_URL",
        "Invalid PNG upload request",
        "Signature mismatch rejection",
        "GITHUB_STEP_SUMMARY",
    ):
        if required not in step_text:
            errors.append(f"workflow is missing required media signature smoke behavior: {required}")
    ...
    for required in (
        ...
        "MEDIA_FILE_SIGNATURE_MISMATCH",
        "staging-media-signature-invalid.png",
        "not-a-png-signature-for-staging-media-signature-smoke",
        "owned_media_count_after == owned_media_count_before",
        "STAGING_ADMIN_EMAIL",
        "STAGING_ADMIN_PASSWORD",
    ):
        if required not in script_text:
            errors.append(f"script is missing required media signature smoke behavior: {required}")
...
mutated_script = script_text.replace("MEDIA_FILE_SIGNATURE_MISMATCH", "INVALID_REQUEST")
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect missing signature mismatch assertion")
...
print("staging media signature smoke workflow policy validation passed")
PY

log "Staging media signature smoke workflow validation passed"
