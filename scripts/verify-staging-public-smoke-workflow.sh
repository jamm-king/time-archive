...
log() {
  printf '[verify-staging-public-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-staging-public-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-staging-public.yml"
SCRIPT="$ROOT_DIR/scripts/verify-staging-public-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "staging-public-smoke":
        errors.append("workflow must serialize the staging public smoke group")
    ...
    for required in (
        "verify-staging-public-smoke.sh",
    ...
    for required in (
        ...
        if required not in script_text:
            errors.append(f"script is missing required public smoke behavior: {required}")

    for forbidden in ("POST ", "PUT ", "PATCH ", "DELETE ", "--request POST"):
        if forbidden in script_text:
            errors.append("script must not use mutating HTTP methods")
...
mutated_script = script_text + "\ncurl --request POST https://staging.example.invalid\n"
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect mutating method")

print("staging public smoke workflow policy validation passed")
PY

log "Staging public smoke workflow validation passed"
