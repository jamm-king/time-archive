...
log() {
  printf '[verify-staging-auth-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-staging-auth-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-staging-auth.yml"
SCRIPT="$ROOT_DIR/scripts/verify-staging-auth-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "staging-auth-smoke":
        errors.append("workflow must serialize the staging auth smoke group")
    if as_bool(concurrency.get("cancel-in-progress")):
        errors.append("workflow must not cancel an in-progress auth smoke check")
    ...
    for required in (
        "verify-staging-auth-smoke.sh",
        "INPUT_PUBLIC_BASE_URL",
        "STAGING_PUBLIC_BASE_URL",
        "GITHUB_STEP_SUMMARY",
    ):
        if required not in step_text:
            errors.append(f"workflow is missing required auth smoke behavior: {required}")
    ...
    for required in (
        ...
        if required not in script_text.lower():
            errors.append(f"script is missing required auth smoke behavior: {required}")

    if re.search(r"AUTH_PASSWORD=.*[A-Z0-9_]{20,}", script_text):
        errors.append("script must not contain generated-looking credentials")
...
mutated_script = script_text.replace("without-csrf", "with-csrf")
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect missing CSRF rejection check")

print("staging auth smoke workflow policy validation passed")
PY

log "Staging auth smoke workflow validation passed"
