...
log() {
  printf '[verify-production-image-publish-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-production-image-publish-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/publish-production-images.yml"
...
def validate(workflow):
    ...
    if concurrency.get("group") != "production-image-publication":
        errors.append("workflow must serialize the production publication group")
    ...
    if "staging" in raw_workflow.lower():
        errors.append("production image workflow must not reference staging")
    ...
    if len(credentials_steps) != 1:
        ...
        if inputs.get("role-to-assume") != "${{ env.AWS_PRODUCTION_IMAGE_PUBLISH_ROLE_ARN }}":
            errors.append("workflow must assume only the configured production image role")
    ...
    expected_env = {
        ...
        "AWS_PRODUCTION_IMAGE_PUBLISH_ROLE_ARN": "${{ vars.AWS_PRODUCTION_IMAGE_PUBLISH_ROLE_ARN }}",
...
print("production image publication workflow policy validation passed")
PY

log "Production image publication workflow validation passed"
