...
log() {
  printf '[verify-production-admin-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-production-admin-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-production-admin.yml"
SCRIPT="$ROOT_DIR/scripts/verify-production-admin-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "production-admin-smoke":
        errors.append("workflow must serialize the production admin smoke group")
    if as_bool(concurrency.get("cancel-in-progress")):
        errors.append("workflow must not cancel an in-progress admin smoke check")

    env = workflow.get("env", {})
    if env != {"PRODUCTION_PUBLIC_BASE_URL": "${{ vars.PRODUCTION_PUBLIC_BASE_URL }}"}:
    ...
    if secret_refs != {"PRODUCTION_ADMIN_EMAIL", "PRODUCTION_ADMIN_PASSWORD"}:
    ...
    for required in (
        "verify-production-admin-smoke.sh",
        "INPUT_PUBLIC_BASE_URL",
        "PRODUCTION_PUBLIC_BASE_URL",
        "GITHUB_STEP_SUMMARY",
    ):
        if required not in step_text:
            errors.append(f"workflow is missing required admin smoke behavior: {required}")
    ...
    for required in (
        ...
        if required not in script_text:
            errors.append(f"script is missing required admin smoke behavior: {required}")

    for forbidden in ("/approve", "/reject", "/hide", "/preview-url"):
        if forbidden in script_text:
            errors.append(f"script must not exercise mutating media admin endpoint: {forbidden}")
    if re.search(r"(echo|printf|log).*PRODUCTION_ADMIN_PASSWORD", script_text):
...
if not validate(mutated, script_text):
    raise SystemExit("policy self-test failed to detect production environment removal")
...
mutated_script = script_text.replace("403", "200")
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect missing regular-user rejection")

print("production admin smoke workflow policy validation passed")
PY

log "Production admin smoke workflow validation passed"
