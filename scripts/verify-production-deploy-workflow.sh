...
log() {
  printf '[verify-production-deploy-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-production-deploy-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}
...
def validate(workflow):
    ...
    if concurrency.get("group") != "production-deployment":
        errors.append("workflow must serialize the production deployment group")
    ...
    if job.get("environment") != "production":
        errors.append("deploy job must use the production GitHub environment")
    ...
    if "staging" in raw_workflow.lower():
        errors.append("production deploy workflow must not reference staging")
    ...
    if len(credentials_steps) != 1:
        ...
        if inputs.get("role-to-assume") != "${{ env.AWS_PRODUCTION_DEPLOY_ROLE_ARN }}":
            errors.append("workflow must assume only the production deploy role")
    ...
    expected_env = {
        ...
        "AWS_PRODUCTION_DEPLOY_ROLE_ARN": "${{ vars.AWS_PRODUCTION_DEPLOY_ROLE_ARN }}",
        "PRODUCTION_INSTANCE_ID": "${{ vars.PRODUCTION_INSTANCE_ID }}",
        "ECR_REGISTRY": "${{ vars.AWS_ACCOUNT_ID }}.dkr.ecr.${{ vars.AWS_REGION }}.amazonaws.com",
        "API_REPOSITORY_NAME": "time-archive-production-api",
        "WEB_REPOSITORY_NAME": "time-archive-production-web",
    }
...
self_tests.append(("production environment removal", mutated))
...
mutated = copy.deepcopy(workflow)
mutated["env"]["PRODUCTION_INSTANCE_ID"] = "i-0123456789abcdef0"
self_tests.append(("literal instance id", mutated))

mutated = copy.deepcopy(workflow)
mutated["env"]["API_REPOSITORY_NAME"] = "time-archive-staging-api"
self_tests.append(("staging repository reference", mutated))

for name, candidate in self_tests:
    if not validate(candidate):
        raise SystemExit(f"policy self-test failed to detect {name}")

print("production deployment workflow policy validation passed")
PY

log "Production deployment workflow validation passed"
