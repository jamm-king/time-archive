...
PAYMENT_REFERENCE="${PAYMENT_REFERENCE:-pi_media_local_${START_SECOND}_${END_SECOND}}"
REQUEST_ID="${REQUEST_ID:-media-local-request-${START_SECOND}-${END_SECOND}}"
EVENT_TYPE="${EVENT_TYPE:-payment_intent.succeeded}"
PAYLOAD_HASH="${PAYLOAD_HASH:-sha256-media-local-${START_SECOND}-${END_SECOND}}"
HEALTH_TIMEOUT_SECONDS="${HEALTH_TIMEOUT_SECONDS:-120}"
UPLOAD_FILENAME="${UPLOAD_FILENAME:-time-archive-upload.png}"
...
log() {
  printf '[verify-media] %s\n' "$1"
}

fail() {
  printf '[verify-media] ERROR: %s\n' "$1" >&2
  exit 1
}
...
payload = (
    b"\x89PNG\r\n\x1a\n"
    b"time-archive-local-upload-verification\n"
...
[[ "$duplicate_media_asset_id" == "$media_asset_id" ]] || fail "Duplicate completion returned a different media asset"
...
log "Local media upload flow verification passed"
