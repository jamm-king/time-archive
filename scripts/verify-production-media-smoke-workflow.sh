...
log() {
  printf '[verify-production-media-smoke-workflow] %s\n' "$1"
}

fail() {
  printf '[verify-production-media-smoke-workflow] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKFLOW="$ROOT_DIR/.github/workflows/smoke-production-media.yml"
SCRIPT="$ROOT_DIR/scripts/verify-production-media-smoke.sh"
...
def validate(workflow, script_text):
    ...
    if concurrency.get("group") != "production-media-smoke":
        errors.append("workflow must serialize the production media smoke group")
    ...
    for required in (
        "verify-production-media-smoke.sh",
    ...
    for required in (
        ...
        "/api/admin/media/assets/$MEDIA_ASSET_ID/approve",
    ...
    for forbidden in ("/reject", "/hide", "/api/internal/payments/fake"):
        if forbidden in script_text:
            errors.append(f"script must not exercise forbidden endpoint: {forbidden}")
    for forbidden in ("preview_url", "upload_url", "playback_url"):
    ...
    if re.search(r"(echo|printf|log).*(PRODUCTION_ADMIN_PASSWORD|PRODUCTION_MEDIA_OWNER_PASSWORD)", script_text):
...
mutated_script = script_text + "\ncurl /api/admin/media/assets/$MEDIA_ASSET_ID/reject\n"
if not validate(workflow, mutated_script):
    raise SystemExit("policy self-test failed to detect forbidden rejection endpoint")

print("production media smoke workflow policy validation passed")
PY

log "Production media smoke workflow validation passed"
