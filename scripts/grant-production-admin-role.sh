...
log() {
  printf '[grant-production-admin-role] %s\n' "$1"
}

fail() {
  printf '[grant-production-admin-role] ERROR: %s\n' "$1" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: scripts/grant-production-admin-role.sh --expected-account-id ID --email EMAIL [options]

Options:
  --expected-account-id ID  Required 12-digit AWS account ID.
  --email EMAIL             Existing production user email to promote to ADMIN.
...
[[ "$AWS_REGION_VALUE" == "ap-northeast-2" ]] ||
  fail "Production admin grant must run in ap-northeast-2"
...
log() {
  printf '[remote-grant-production-admin-role] %s\n' "\$1"
}

fail() {
  printf '[remote-grant-production-admin-role] ERROR: %s\n' "\$1" >&2
  exit 1
}
...
log "Granting ADMIN role to existing production user"
...
log "production admin role grant completed"
REMOTE
...
payload = {
    ...
    "Comment": "Grant Time Archive production ADMIN role",
    "Parameters": {
...
log "production admin role grant succeeded"
