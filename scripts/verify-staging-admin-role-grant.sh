...
log() {
  printf '[verify-staging-admin-role-grant] %s\n' "$1"
}

fail() {
  printf '[verify-staging-admin-role-grant] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SCRIPT="$ROOT_DIR/scripts/grant-staging-admin-role.sh"
...
if "Comment\": \"Grant Time Archive staging ADMIN role\"" not in script:
    errors.append("SSM command comment must identify the audited operation")

if errors:
    raise SystemExit("\n".join(f"- {error}" for error in errors))

print("staging admin role grant policy validation passed")
PY

log "Staging admin role grant validation passed"
