...
log() {
  printf '[verify-production-owned-range-grant] %s\n' "$1"
}

fail() {
  printf '[verify-production-owned-range-grant] ERROR: %s\n' "$1" >&2
  exit 1
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SCRIPT="$ROOT_DIR/scripts/grant-production-owned-range.sh"
...
required = [
    ...
    "/time-archive/production/",
    "/time-archive/production/database/username",
...
if "Unexpected runtime parameter path: $RUNTIME_PARAMETER_PATH" not in script:
    errors.append("script must validate the production runtime parameter path")

if "Comment\": \"Grant Time Archive production owned range\"" not in script:
    errors.append("SSM command comment must identify the audited operation")

if errors:
    raise SystemExit("\n".join(f"- {error}" for error in errors))

print("production owned range grant policy validation passed")
PY

log "production owned range grant validation passed"
