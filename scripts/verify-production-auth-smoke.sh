...
log() {
  printf '[verify-production-auth-smoke] %s\n' "$1"
}

fail() {
  printf '[verify-production-auth-smoke] ERROR: %s\n' "$1" >&2
  exit 1
}

usage() {
  cat <<'USAGE'
Usage: scripts/verify-production-auth-smoke.sh [--base-url URL]

Verifies the deployed production HTTPS authentication flow. The check creates a
disposable production user with a production-auth-smoke email prefix.
...
AUTH_DISPLAY_NAME="${AUTH_DISPLAY_NAME:-Production Auth Smoke}"
...
print("production-auth-smoke-%s@example.com" % uuid.uuid4().hex)
')"
  export AUTH_EMAIL
fi
...
log "Registered disposable production user"
...
log "Production auth smoke check passed"
