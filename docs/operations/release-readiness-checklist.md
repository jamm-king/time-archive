...
| Production deployment workflow | Needs verification | Manual production image publication and deployment workflows are prepared, production-specific workflow policy checks run in CI, deployment is gated by the GitHub `production` environment, and [Production First Deploy Runbook](production-first-deploy-runbook.md) defines execution inputs, routing, smoke checks, and failure handling. Run production image publication, production deployment, and production smoke checks before marking this Ready. |
