...
| Presigned upload URLs | Needs verification | A manual staging presigned upload CORS smoke workflow now verifies upload request creation, CORS preflight for `PUT` with `content-type`, and actual presigned `PUT` response CORS headers from the deployed Web origin. Run it against staging before marking this gate ready. |
