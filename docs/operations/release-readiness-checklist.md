...
| Payment idempotency | Ready for staging | Backend PayPal tests cover capture retry after failure, already-captured retry, duplicate webhook `alreadyProcessed` propagation, non-completed capture rejection, and capture/order/amount/currency mismatch rejection. The [PayPal Staging Idempotency Drill](paypal-staging-idempotency-drill.md) passed for resent Sandbox event `WH-7HU60164371570724-75J73461KN145691Y`: CloudWatch showed verified webhook handling and HTTP `200`, `payment_events` remained one `PROCESSED` row, and the latest PayPal purchase/ownership records remained single active records. Repeat after PayPal webhook, payment finalization, or deployment routing changes. |
| Production PayPal live setup | Blocked for production | Follow [Production PayPal Live Setup](production-paypal-live-setup.md): provision a dedicated production PayPal Live app, live webhook URL, production SSM parameters, first live low-value payment drill, refund/rollback procedure, and production Dashboard reconciliation before enabling paid production traffic. |
...
| Docker images | Needs verification | ARM64 builds pass CI, staging images publish to ECR, and manual production image publication workflow is prepared with immutable full Git SHA tags, provenance, SBOM, and digest verification. Run production image publication and review ECR scan findings before production deployment. |
...
| Production secret injection | Ready | Production runtime SSM parameters are provisioned, live metadata validation passed without decrypting values, the production application role is limited to production parameter reads, production DB user bootstrap passed, and EC2 runtime rendering created `/run/time-archive/runtime.env` with mode `0600` without printing secret values. |
...
| Production deployment workflow | Needs verification | Manual production image publication and deployment workflows are prepared, production-specific workflow policy checks run in CI, deployment is gated by the GitHub `production` environment, and [Production First Deploy Runbook](production-first-deploy-runbook.md) defines execution inputs, routing, smoke checks, and failure handling. Production public, security-header, and auth smoke scripts passed locally against `https://time-archive.com`; run the matching GitHub Actions workflows from `main` before marking this Ready. |
...
- [Production PayPal Live Setup](production-paypal-live-setup.md)
...
and incident response plan. Real-money launch also requires production PayPal
live verification, production runtime
