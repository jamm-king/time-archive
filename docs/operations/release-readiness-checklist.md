...
| Production secret injection | Needs verification | Production parameter contract and safety boundaries are documented in [Production Runtime Parameters](production-runtime-parameters.md), and CI validates the committed production runtime parameter contract. Provision production-scoped parameters, IAM access, KMS policy, live SSM metadata, and runtime rendering verification before marking this Ready. |
